...
list1 = [] 
n = int(input("Nhap vao 1 so nguyen la so phan tu cua 1 list : "))

for i in range(1 , n+1 ) :
    number = int( input( f'Nhap vao phan tu thu {i} : ' ) )
    list1.append(number) 
print( list1 ) 
print()
...
X = int(input("Nhap vao mot so X tu ban phim : ")) 

count_SoLanXuatHien = 0 

for number in list1 :
    if ( number == X ) : 
        count_SoLanXuatHien+=1 
    else:
        continue

print( f"So lan xuat hien cua {X} trong list la : " , count_SoLanXuatHien) 
print() 
...
list1.sort() 
print(list1) 
print( f'So be nhat trong list la : ' ,list1[0]) 
print( f'So lon nhat trong list la : ' ,list1[-1])
print()

# -Nhập một số Y tùy chọn từ bàn phím. Chèn số Y vào đầu list.
Y = int( input('Nhap mot so Y tuy chon tu ban phim : '))
list1.insert(0 , Y) 
print(list1) 
print()
